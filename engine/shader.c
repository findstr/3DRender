...
void
shader_anim_vert(struct shader_appdata *a2v, struct shader_v2f *v2f, const struct shader_global *G)
{
	float time = G->TIME;
	vector4_t delta;
	vector4_t pos = a2v->position;
	vector4_init(&delta, 0.f, sinf(pos.x / 2 + time * 3.14f) * 5.0f , 0.f);
	pos.x += delta.x;
	pos.y += delta.y;
	pos.z += delta.z;
...
rgba_t
shader_anim_frag(struct shader_v2f *v2f, const struct shader_global *G)
{
	return texture_sample(G->BITMAP, &v2f->texcoord0);
}
