...
void
object_polynormals2(struct object *obj)
{
	int i;
	int v0, v1, v2;
	vector3_t u,v,n;
	...
	for (i = 0; i < obj->tri_num; i++) {
		...
		v1 = p->vert[1];
		v2 = p->vert[2];
		vector3_sub(&vlist[v1].v2f.sv_position, &vlist[v0].v2f.sv_position, &u);
		vector3_sub(&vlist[v2].v2f.sv_position, &vlist[v0].v2f.sv_position, &v);
		vector3_cross(&u, &v, &n);
		vector4_init(&p->normal_local, n.x, n.y, n.z);
	}
	return ;
