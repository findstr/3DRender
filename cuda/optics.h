...
d_ggx_tr(const vector3f &n, const vector3f &h, float roughness)
{
	...
	float n_dot_h = fmaxf(n.dot(h), 0.f);
	...
	const vector3f &l, float roughness)
{
	...
	float k = (r*r) / 8.f;
	float n_dot_v = fmaxf(n.dot(v), 0.f);
	float n_dot_l = fmaxf(n.dot(l), 0.f);
...
f_schlick(const vector3f &v, const vector3f &l, const vector3f &f0)
{
	...
	float h_dot_v = 1.f - fmaxf(h.dot(v), 0.f);
	return f0 + (vector3f(1.f, 1.f, 1.f) - f0) * powf(h_dot_v, 5.f);
}
